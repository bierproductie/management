...
\begin{table}[ht]
    \begin{tabularx}{\textwidth}{|>{\RaggedRight}p{4cm}|>{\RaggedRight}X|}
        \hline
        \textbf{Problem} & The production it not efficient enough fort the brewery
        to produce large amount of beers in good quality\\
        \hline
        \textbf{Problem Statement} & How to optimize the brewery, in order to 
        produce large amount of quality beers quickly\\
        \hline
        \textbf{Related questions} & 
            \begin{itemize}
                \item Which some thing can be use to optimize the production?
                \item How to use the concepts of calculus and linear algebra, to
                    statistics about the machine
                \item How can we create a web fronend ish?
                \item How to make Separation of concerns? Web, network and docker?
            \end{itemize}
        \\ 
        \hline
    \end{tabularx}
    \caption{Problem statement showcase, with the elicited problem at the top, 
    the statement below it, and related questions the group wants to answer} 
