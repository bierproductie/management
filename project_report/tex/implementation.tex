...
\subsection{Database}
The Database implementation is just a database that comes from the premade software 
shelf made by others. The database is a PostgreSQL relational database, it is
tried and battle tested. The creating of tables and updates of those tables is 
handle by the REST API. The REST API has been taking inspiration from the EER 
Diagram see Figure \ref{figure:eer_diagram} for how the tables could be structured.
