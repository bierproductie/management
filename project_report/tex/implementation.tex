...
the database in the production setup is routed trough the REST API, this way
we ensure data quality and easier error handling. But also from a security
standpoint this is superior since we can make an internal docker network that
is only shared with the REST API, which ensures that none other than the REST
API can even connect to it. The database consists of the following tables
\textt{data_over_time}, \texttt{recipes}, \texttt{batches} and
\texttt{alembic_version}. The \texttt{data_over_time} table is where live data
is posted to by the client, to improve performance this table is partioned on a
monthly basis based on the measurement timestamp of each row. Given our system
design, an interesting point to look at is the delay between the machine
...
when data is inserted on the \texttt{data_over_time} table.
