...
it's using the async ORM Gino. The source code structure is inspired from best
...
Another part of implementation is the development setup, via our continous
integration pipeline we check if the code is linted properly as well as unit
and system tests are passing, if one of these steps are failing the pull request
is not allowed to be merged to master. When new branches are merged to master,
the last part of our pipeline is being run which builds a new docker image to
our private registry which is then deployed automatically to 
https://api.bierproductie.nymann.dev/docs, this can be verified via the version
matching the latest commit hash.
