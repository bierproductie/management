...
\begin{table}[H]
    \begin{tabularx}{\textwidth}{|>{\RaggedRight}p{1cm}|>{\RaggedRight}p{4cm}|>{\RaggedRight}X|>{\RaggedRight}c|}
        \hline
        \textbf{ID} & \textbf{Name} & \textbf{Description} & \textbf{Priority} \\
        \hline
        R01 & Control production line & Control the brewery's production & M \\
        \hline
        R02 & Monitor production & Monitor data from the production line & M \\
        \hline
        R03 & Monitor production & Store the collected data for further analysis & M \\
        \hline
        R04 & Administer batches & Keep track of produced batches (batch ID) & M \\
        \hline
        R05 & Store batch info & Collect various data associated with current batch number from the machine & M \\
        \hline
        R06 & Batch report & Produce a batch report (PDF/dashboard style format) & M \\
        \hline
        R07 & Live data & Monitor and display live relevant data from the machine & M \\
        \hline
        R08 & Documentation & Documentation must contain an illustration that defines the different components in the setup in relation to the ISA88\^{}1213 Part 1 Physical Hierarchy model & M \\
        \hline
        R09 & Visualisation & Visualisation that can be accessed and used to display the production data & M \\
        \hline
...
\subsubsection{Supplementary Requirements}
In addition to the requirements in Table \ref{table:Requirements}, FURPS+ method
has been used to identify supplementary requirements in order to measure the
quality of the software. The table contains a description of Functional and
...
Based on the found requirements, the system's use cases can be found. Use cases
are written descriptions of how a user will perform different tasks on a system.
They outline a systems behaviour as it responds to a request from the user. Each
use case is represented as a sequence of steps, beginning with the users goal
and ending when it that goal is fulfilled. \\
...
\begin{table}[H]
    \begin{tabularx}{\textwidth}{|>{\RaggedRight}X|}
		...
		\hline
        \textbf{post conditions:} The beer production process has been started \\
        \hline
...
In order to visualise the use cases, a use case diagram can be made. The diagram
can be seen in Figure \ref{figure:ucdiagram}. The diagram shows the outline of
the MES (the box), as well as which use cases each of the external actors can
interact with.
